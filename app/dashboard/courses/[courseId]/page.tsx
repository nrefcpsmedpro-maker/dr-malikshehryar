...
export default async function StudentCourseOverview({
  ...
  const [{ data: profile }, { data: enrollment }, { data: course, error }, { data: results }] = await Promise.all([
    supabase
      .from('profiles')
      .select('role')
      .eq('id', user.id)
  ...
  if ((profile?.role !== 'admin' && !enrollment) || error || !course) {
    redirect('/dashboard');
  }

  const subjects = [...(course.subjects ?? [])].sort((a, b) => a.order_index - b.order_index);
      ...
      {course.mock_tests && course.mock_tests.length > 0 && (
        <div className="pt-8 mt-8 border-t border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[...course.mock_tests].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()).map((test) => {
